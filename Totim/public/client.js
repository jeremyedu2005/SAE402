...
//==============================================
// OBSTACLES — dessinés une seule fois après réception du premier game_state
//==============================================
const divObstacles = [];
let obstaclesRendus = false;

function dessinerObstacles(obstacles) {
    if (obstaclesRendus) return;
    obstaclesRendus = true;

    // Un seul SVG couvrant toute l'arène, un <polygon> par obstacle
    ...
    for (const o of obstacles) {
        ...
        // pts = [[x1,y1],[x2,y2],...] → "x1,y1 x2,y2 ..."
        poly.setAttribute("points", o.pts.map(([x, y]) => `${x},${y}`).join(" "));
        poly.setAttribute("fill",         "#4a4a4a");
    ...
    divObstacles.push(svg); // stocké pour référence future
}
...
document.addEventListener("mousemove", (e) => {
    ...
    // Mise à jour des coordonnées arène en temps réel
    const pos = ecranVersArene(e.clientX, e.clientY);
    coordDisplay.textContent = `x: ${Math.round(pos.x)}  y: ${Math.round(pos.y)}`;
});

//==============================================
// AFFICHEUR DE COORDONNÉES CURSEUR
// Affiche la position du curseur dans l'espace de l'arène
// → utile pour placer les obstacles dans OBSTACLES[]
//==============================================
...
// SOCKET — affichage
//==============================================
socket.on("game_state", ({ joueurs, projectiles, zones, zonesPersistantes, telegraphes, obstacles }) => {
    if (obstacles?.length) dessinerObstacles(obstacles);
