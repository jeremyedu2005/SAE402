...
//
// Chaque obstacle est défini par ses sommets : { pts: [[x1,y1],[x2,y2],[x3,y3],[x4,y4]] }
// Les points doivent être dans l'ordre (horaire ou anti-horaire) et le polygone doit être CONVEXE.
// 4 points = quadrilatère quelconque, mais fonctionne aussi avec 3 points (triangle) ou plus.
//
// Exemples :
...
//==============================================
// SAT (Separating Axis Theorem) — collision polygone convexe
//==============================================

// Projette un tableau de points sur un axe (nx, ny) et retourne [min, max]
...
// Retourne les axes normaux d'un polygone convexe
function axesPolygone(pts) {
    ...
    for (let i = 0; i < pts.length; i++) {
        ...
        axes.push([-edgeY / len, edgeX / len]); // normale perpendiculaire
    }
    return axes;
}

// Teste si un polygone A et un polygone B se chevauchent (SAT)
function polygonesSeChevauchent(ptsA, ptsB) {
    const axes = [...axesPolygone(ptsA), ...axesPolygone(ptsB)];
    for (const [nx, ny] of axes) {
        const [minA, maxA] = projeterPolygone(ptsA, nx, ny);
        const [minB, maxB] = projeterPolygone(ptsB, nx, ny);
        if (maxA < minB || maxB < minA) return false; // axe séparateur trouvé
    }
    return true; // aucun axe séparateur → collision
}

// Convertit le rectangle du joueur (px, py, taille) en polygone de points
function rectEnPts(px, py, taille) {
    return [[px, py], [px + taille, py], [px + taille, py + taille], [px, py + taille]];
}

// Convertit un cercle en octogone approché (suffisant pour la précision voulue)
...
// Teste si le rectangle du joueur (px, py, CHAR_SIZE) touche un obstacle
function collisionObstacle(px, py, pw, ph) {
    const rectJoueur = rectEnPts(px, py, pw);
    for (const o of OBSTACLES) {
        if (polygonesSeChevauchent(rectJoueur, o.pts)) return true;
    }
    return false;
}

// Teste si un cercle (projectile) touche un obstacle
...
function broadcastGameState() {
    io.emit("game_state", {
        ...
        obstacles:         OBSTACLES,   // statiques — le client les reçoit à chaque state
    });
}
...
function serverGameLoop() {
    ...
    for (const [socketId, keys] of Object.entries(keysPressed)) {
        ...
        // Déplacement séparé X et Y pour glisser le long des murs
    ...
    for (const [pid, proj] of Object.entries(projectiles)) {
        ...
        // Collision avec un obstacle : détruit le projectile (explose si applicable)
