...
//==============================================
//==============================================
// PROJECTILES (Version Animée)
//==============================================
function creerElementProjectile(proj) {
    const div = document.createElement("div");
    div.dataset.projId = proj.id;

    // Ajustement taille pour contenir une frame (3x moins large)
    // On conserve la proportion de votre spritemap (largeur totale = 3 * hauteur)
    const frameHeight = proj.taille * 4;
    const frameWidth = frameHeight; // comme la spritemap est 3x1, une frame est carrée

    if (proj.sprite) {
        // Si c'est la boule de feu, on applique l'animation
        const estBouleDeFeu = proj.sprite === "bouledefeu.png";
        ...
        if (estBouleDeFeu) {
            ...
            // Pour les autres sprites qui ne sont pas des spritemaps 3x1
        ...
        div.style.cssText = `
            position: absolute;
            width: ${proj.taille * 4}px;
            height: ${proj.taille * 4}px;
            border-radius: 50%;
    ...
    // Gestion de la rotation (déjà présente dans votre code)
